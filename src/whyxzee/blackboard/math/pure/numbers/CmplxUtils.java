...
public class CmplxUtils {
     ...
     * 
     * 
     * @param argPow A real number. Cannot be Double.POSITIVE_INFINITY,
     *               Double.NEGATIVE_INFINITY, nor Double.NaN.
     * @return
     */
    private static final Complex power(Complex base, Object argPow) {
        BNum power = BNum.fromObj(argPow);
        ...
        if (base.isReal()) {
            ...
            if (!power.isRational()) {
                ...
                return Complex.fromPolar(ComplexType.COMPLEX, a.negate().power(power),
                        (Math.PI * power.getValue()));
            }

            if (!NumberUtils.inOpenRange(power.getValue(), 0, 1)) {
            ...
            int[] ratio = power.findRatio();
            if (ratio[1] % 2 == 1) {
                // negative base doesn't work for some reason
                base.setA(-Math.pow(-a.getValue(), power.getValue()));
                base.calcPolar();
                return base;

            } else if (ratio[1] == 2) {
                base.setB(Math.pow(-a.getValue(), power.getValue()));
                base.setA(0);
                base.calcPolar();
                return base;
            }

            return Complex.fromPolar(ComplexType.COMPLEX, a.negate().power(power), Math.PI * power.getValue());

        } else if (base.isImaginary() && power.isInteger()) {
            // multiplying by an imaginary number is rotating the number by PI / 2 rads on
            // the imaginary-real axis.

            if (power.mod(4) == 0) {
                // double check this
                base.setA(b.power(power));
                base.setB(0);

            } else if (power.mod(2) == 0) {
                base.setA(b.power(power).negate());
                base.setB(0);

            } else if (power.mod(4) == 3) {
                base.setA(0);
                base.setB(b.power(power).negate());
